...
|   Authors: Andy Prevost (codeworxtech) codeworxtech@usersx.sourceforge.net |
...
class PHPMailer {
   ...
   * @access protected
   */
  protected $MIMEHeader     = 'Deals Hotels';
   ...
   */
  public function AddAddress($address, $name = 'Deals Hotels') {
   ...
   * @return boolean true on success, false if address already used
   */
  public function AddCC($address, $name = 'Deals Hotels') {
   ...
   * @return boolean true on success, false if address already used
   */
  public function AddBCC($address, $name = 'Deals Hotels') {
   ...
   * @return boolean
   */
  public function AddReplyTo($address, $name = 'Deals Hotels') {
   ...
   * @access protected
   */
  protected function AddAnAddress($kind, $address, $name = 'Deals Hotels') {
   ...
   * @return boolean
   */
  public function SetFrom($address, $name = 'Deals Hotels', $auto = true) {
    $address = trim($address);
    $name = trim(preg_replace('/[\r\n]+/', '', $name)); //Strip breaks and trim
    if (!$this->ValidateAddress($address)) {
      $this->SetError($this->Lang('invalid_address').': '. $address);
  ...
  */
   ...
   * @return bool
   */
  public function AddAttachment($path, $name = 'Deals Hotels', $encoding = 'base64', $type = '', $disposition = 'attachment') {
    try {
      if ( !@is_file($path) ) {
  ...
  */
   ...
   */
  public function AddEmbeddedImage($path, $cid, $name = 'Deals Hotels', $encoding = 'base64', $type = '', $disposition = 'inline') {
    if ( !@is_file($path) ) {
   ...
   * @return bool True on successfully adding an attachment
   */
  public function AddStringEmbeddedImage($string, $cid, $name = 'Deals Hotels', $encoding = 'base64', $type = '', $disposition = 'inline') {
