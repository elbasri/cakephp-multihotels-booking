<?php
	...
	*/
	...
	if ( ! function_exists ('mysqli_connect') ) die('<b>Fatal Error:</b> ezSQL_mysqli requires mySQLi Lib to be compiled and or linked in to the PHP engine');
	if ( ! class_exists ('ezSQLcore') ) die('<b>Fatal Error:</b> ezSQL_mysqli requires ezSQLcore (ez_sql_core.php) to be included/loaded before it can be used');

	class ezSQL_mysqlii extends ezSQLcore
	{
		...
		*  same time as initialising the ezSQL_mysqlii class
		*/

		function ezSQL_mysqlii($dbuser='', $dbpassword='', $dbname='', $dbhost='localhost', $encoding='')
		{
			$this->dbuser = $dbuser;
		...
		*/
		...
		{
			global $ezSQL_mysqlii_str; $return_val = false;
			...
			if ( ! $dbuser )
			{
				$this->register_error($ezSQL_mysqlii_str[1].' in '.__FILE__.' on line '.__LINE__);
				$this->show_errors ? trigger_error($ezSQL_mysqlii_str[1],E_USER_WARNING) : null;
			}
			// Try to establish the server database handle
			else if ( ! $this->dbh = new mysqli($dbhost,$dbuser,$dbpassword) )
			{
				$this->register_error($ezSQL_mysqlii_str[2].' in '.__FILE__.' on line '.__LINE__);
				$this->show_errors ? trigger_error($ezSQL_mysqlii_str[2],E_USER_WARNING) : null;
			}
			else
			{
				$this->dbuser = $dbuser;
		...
		*/
		...
		function select($dbname='', $encoding='')
		{
			...
			if ( ! $dbname )
			{
				$this->register_error($ezSQL_mysqlii_str[3].' in '.__FILE__.' on line '.__LINE__);
				$this->show_errors ? trigger_error($ezSQL_mysqlii_str[3],E_USER_WARNING) : null;
			}

			// Must have an active database connection
			else if ( ! $this->dbh )
			{
				$this->register_error($ezSQL_mysqlii_str[4].' in '.__FILE__.' on line '.__LINE__);
				$this->show_errors ? trigger_error($ezSQL_mysqlii_str[4],E_USER_WARNING) : null;
			}
