...
class PagesController extends Controller {
    ...
    var $uses = array('Ville','Pay', 'Hotel', 'Chambre', 'Reservation', 'Inventaire', 'Banner', 'Featured', 'Slider', 'Promotion', 'Content', 'Guide', 'Activite');
    ...
    function groupes() {
        ...
        if (!empty($this->data)) :
            ...
            if ($this->Groupe->validates()) :
                ...
                $this->_sendEmail('Offre Deals Hotels Groupe', 'groupe', $email);
        ...
        @$metaKeys = "hotel marrakech, hotel maroc, voyage marrakech pas cher, voyage organisé marrakech, voyage organisé maroc";
    ...
    function contact() {
         ...
         */
        $this->Contact->Behaviors->attach('Captcha.Validation');
        ...
        if (!empty($this->data)) :
            ...
            if ($this->Contact->validates()) :
                ...
                $email = Configure::read('resa_mail');
                $this->_sendEmail('Offre Deals Hotels Contact', 'contact', $email);
                $this->Session->setFlash(' Merci ! <br/>Votre message a bien été envoyé, nous vous enverrons une réponse dans les plus brefs délais.', 'default', array('class' => 'flash_sucess'));
            else:
    ...
    function affiliation() {
        ...
        if (!empty($this->data)) :
            ...
            if ($this->Affiliation->validates()) :
                ...
                $this->_sendEmail('Offre Deals Hotels Affiliation', 'affiliation', $email);
                $this->Session->setFlash(' Merci ! <br/>Votre message a bien été envoyé, nous vous enverrons une réponse dans les plus brefs délais.', 'default', array('class' => 'flash_sucess'));
    ...
    function theevents() {
        ...
        if (!empty($this->data)) :
            ...
            if ($this->Event->validates()) :
                ...
                $this->_sendEmail('Offre Deals Hotels Event', 'event', $email);
                $this->Session->setFlash(' Merci ! <br/>Votre message a bien été envoyé, nous vous enverrons une réponse dans les plus brefs délais.', 'default', array('class' => 'flash_sucess'));
    ...
    function events() {
        ...
        @$metaKeys = "mariage maroc, organisation mariage marrakech, mariage marrakech, traiteur mariage marrakech";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
    }
    ...
    function promotions($id = null) {
        if ($id == 4 or $id == 0) {
            ...
            $this->set(compact('promotions', 'sliders', 'banner'));
        } else {
            $this->redirect("http://www.offredealshotels.com/");
        }
    }
    ...
    function content($id) {
        ...
        @$metaKeys = "location voiture marrakech, location voiture marrakech aéroport, location voiture agadir, location voiture casablanca";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
    }
    ...
    function activite() {
		...
		@$metaKeys = "maroc, marrakech, tourisme maroc, Informations pratiques, Informations touristiques";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
    ...
    function vols() {
        ...
        @$metaKeys = "Vol marrakech pas cher, vol pas cher marrakech, paris marrakech pas cher";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
    }

    function home() {
        $featureds = $this->Featured->find('all', array('contain' => array('Hotel', 'Hotel.Ville', 'Hotel.Devise')));
        $sliders = $this->Slider->find('all', array(
            'conditoins' => array('Slider.acccueil' => true),
        ...
        $promotions = $this->Promotion->find('all', array('conditions' => $cond, 'order' => 'rand()', 'group' => 'Promotion.hotel_id', 'limit' => 5, 'contain' => array('Hotel', 'Hotel.Devise', 'Hotel.Ville', 'Chambre')));
        $minPrices = array();
        ...
        @$metaKeys = "hotel marrakech, hotel maroc, hôtel marrakech, hotel maroc, riad maroc, riad marrakech, location villa marrakech, location appartement, voyage organisé, mariage maroc, Vol marrakech, location voiture maroc";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
        //debug($sliders); exit;
        $this->set(compact('featureds', 'promotions', 'sliders', 'minPrices'));
    }
...
$metaDes = "la liste des riads a essaouira, Offre Deals Hotels a selectionner pour vous des riads de luxe pas cher dans toute la region de essaouira ";
        $title_for_layout = 'riad essaouira - liste des riads a essaouira';
        @$metaKeys = "riad, essaouira, riad essaouira, mogador, riyad essaouira, riad luxe essaouira, riad luxe pas cher essaouira, riad maroc, voyage essaouira";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
		$this->data = array();
    ...
    function deals() {
    $metaDes = "les meilleurs deals des hotels a marrakech et dans tout le maroc, Offre Deals Hotels a negocier pour vous les meilleurs deals dans toute le maroc avec des réductions incroyables jusqu'a -70%";
        $title_for_layout = 'deal hotel marrakech - les meilleurs deals au marrakech';
        @$metaKeys = "hotel deal, deal marrakech, marrakech, hotel deal  marrakech, deal riad marrakech, deal maroc, hotel pas cher marrakech, deal vacances, deal maroc, hotel luxe pas cher marrakech, riad luxe pas cher marrakech, riad pas cher maroc, hotel pas cher maroc, deal maroc";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
		$this->data = array();
    ...
    $metaDes = "les offres les moins cher des hotels a marrakech et dans tout le maroc, Offre Deals Hotels vous propose des offres correspondant a tout le monde";
        $title_for_layout = 'hotel petit budget - les hotels les moins cher au maroc';
        @$metaKeys = "hotel petit budget, hotel deal  marrakech, deal riad marrakech, hotel petit budget maroc, hotel moins cher marrakech, riad pas cher, hotel marrakech pas cher, hotel maroc pas cher, hotel luxe pas cher marrakech, riad luxe pas cher marrakech, riad pas cher maroc";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
		$this->data = array();
		$this->data['Search']['HotelType']	= '';
		$this->data['Search']['adulte'] = 0;
...
function marrakech() {
$metaDes = "la liste des hotels a marrakech, Offre Deals Hotels a selectionner pour vous des riads et hotels dans toute la region de marrakech ";
        $title_for_layout = 'hotel marrakech - liste des hotels a marrakech';
        @$metaKeys = "hotel, marrakech, hôtel marrakech, hotel marrakech, riad marrakech, hotel luxe marrakech, riad luxe marrakech, riad pas cher, hotel pas cher marrakech, hotel maroc";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
$this->data = array();
$this->data['Search']['HotelType']	= '';
$this->data['Search']['adulte'] = 0;
...
$metaDes = "la liste des riads a marrakech, Offre Deals Hotels a selectionner pour vous des riads de luxe pas cher dans toute la region de marrakech avec des réductions incroyables";
        $title_for_layout = 'riad marrakech - liste des riadss a marrakech';
        @$metaKeys = "riad, marrakech, ryad, marrakesh, riad marrakech, ryad marrakech, riad marrakesh, riad luxe marrakech, riad pas cher, riad maroc";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
$this->data = array();
$this->data['Search']['HotelType']	= '5';
$this->data['Search']['adulte'] = 0;
...
$metaDes = "la liste des appartement a marrakech, Offre Deals Hotels a selectionner pour vous des appartements de luxe pas cher pour votre séjour dans toute la region de marrakech avec des reductions incroyables ";
        $title_for_layout = 'appartement a marrakech - liste des appartements a marrakech';
        @$metaKeys = "appart, appartement, marrakech, marrakesh, appartement marrakech, appart marrakech, appart marrakesh, appartement luxe marrakech, appart luxe marrakech, appartement pas cher, appartement pas cher marrakech, appartement haut atlas, appartement atlas, appartement maroc";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
$this->data = array();
...
function riadfes() {
$metaDes = "la liste des riads a fes, Offre Deals Hotels a selectionner pour vous des riads de luxe pas cher  dans toute la region de fes avec des réductions incroyables";
        $title_for_layout = 'riads a fes - liste des riads a fes';
        @$metaKeys = "riad, ryad, fes, riad fes, ryad fes, riad fez, riad luxe fes, ryad luxe fes, riad pas cher, riad pas cher fes, hotel moulay yacoub, appartement moulay yacoub, riad maroc";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
$this->data = array();
$this->data['Search']['HotelType']	= '5';
$this->data['Search']['adulte'] = 0;
...
$metaDes = "la liste des appartement a fes, Offre Deals Hotels a selectionner pour vous des appartements de luxe pas cher pour votre séjour dans toute la region de fes avec des prix trés raisonables ";
        $title_for_layout = 'appartement a fes - liste des appartements a fes';
        @$metaKeys = "appart, appartement, fes, appartement fes, appart fes, appart fez, appartement luxe fes, appart luxe fes, appartement pas cher, appartement pas cher fes, appartement sidi harazem, appartement moulay yacoub, appartement maroc";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
$this->data = array();
$this->data['Search']['HotelType']	= '4';
$this->data['Search']['adulte'] = 0;
...
function casablanca() {
$metaDes = "la liste des hotels a casablanca, Offre Deals Hotels a selectionner pour vous des riads et hotels a casablanca avec des reductions jusqu'a -60%";
        $title_for_layout = 'hotel casablanca - liste des hotels a casablanca';
        @$metaKeys = "hotel, casablanca, hôtel casablanca, hotel casablanca, hotel luxe casablanca, riad casablanca, hotel maroc, salon casablanca, affaires casablanca";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
		$this->data = array();
		$this->data['Search']['HotelType']	= '';
		$this->data['Search']['adulte'] = 0;
...
function agadir() {
$metaDes = "la liste des hotels a agadir, Offre Deals Hotels a selectionner pour vous des riads et hotels dans toute la region d'agadir ";
        $title_for_layout = 'hôtels a agadir - reservez votre hotel a agadir';
        @$metaKeys = "hotel, agadir, hôtel agadir, hotel luxe agadir, plage agadir, hotel agadir, riad agadir, hotel maroc, voyage agadir, surf agadir";
         $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
		$this->data = array();
		$this->data['Search']['HotelType']	= '';
		$this->data['Search']['adulte'] = 0;
...
function tanger() {
$metaDes = "la liste des hotels a tanger, Offre Deals Hotels a selectionner pour vous des riads et hotels dans toute la region de tanger ";
        $title_for_layout = 'hotel tanger - liste des hotels a tanger';
        @$metaKeys = "hotel, tanger, hôtel tanger, hotel luxe tanger, plage tanger, hotel tanger, hotel nord maroc, hotel maroc, voyage tanger, surf tanger";
         $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
		$this->data = array();
		$this->data['Search']['HotelType']	= '';
		$this->data['Search']['adulte'] = 0;
...
function essaouira() {
$metaDes = "la liste des hotels a essaouira, Offre Deals Hotels a selectionner pour vous des riads et hotels dans toute la region de essaouira ";
        $title_for_layout = 'hotel essaouira - liste des hotels a essaouira';
        @$metaKeys = "hotel, essaouira, hôtel essaouira, mogador, hotel essaouira, riad essaouira, hotel luxe essaouira, riad luxe essaouira, hotel maroc, voyage essaouira";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
		$this->data = array();
		$this->data['Search']['HotelType']	= '';
		$this->data['Search']['adulte'] = 0;
...
function fes() {
$metaDes = "la liste des hotels a fes, Offre Deals Hotels a selectionner pour vous des riads et hotels dans toute la region de fes ";
        $title_for_layout = 'hotels a fes - liste des hotels a fes';
        @$metaKeys = "hotel, fes, hôtel fes, hotel fes, riad fes, hotel luxe fes, riad luxe fes, riad pas cher, hotel pas cher fes, hotel moulay yacoub, appartement moulay yacoub, hotel maroc";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
$this->data = array();
$this->data['Search']['HotelType']	= '';
$this->data['Search']['adulte'] = 0;
...
$this->etablissement();
    ...
    function villas() {
        ...
        $metaDes = "Profitez des meilleures deals pour location de villa à Marrakech ou Agadir à des prix pas cher pour passer votre séjour au Maroc.";
        $title_for_layout = "Réservation ou location pas cher de villa à Marrakech et Agadir";
        @$metaKeys = "villa marrakech, location villa marrakech, location villa agadir, location villa marrakech pas cher";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
    ...
    function appart() {
        ...
        @$metaKeys = "hotel maroc, appart marrakech, location appartement marrakech, location appartement martil, location appartement tetouan, location vacances marrakech, location vacances agadir";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
    ...
    function hotels() {
        ...
        //$this->set(compact('metaDes'));
        $metaDes = "Profitez des meilleures deals sur les hôtels de luxe à marrakech ou agadir à des prix pas cher pour passer vos vacances au Maroc.";
        $title_for_layout = 'Reservez un hotel de luxe pas cher a Marrakech ou Agadir';
        @$metaKeys = "hotel marrakech, hotel maroc, hotel agadir, booking maroc, booking marrakech, hotel essaouira, hotel luxe marrakech, hotel marrakech pas cher, hotel agadir pas cher, luxury hotel marrakech, riad maroc";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
    }
    ...
    function riads() {
        ...
        @$metaKeys = "riad marrakech, riad essaouira, riad fes, riad maroc, riad marrakech pas cher, riad luxe marrakech, location riad marrakech, séjour marrakech pas cher, riad marrakech medina, luxury riad marrakech, guest house marrakech";
        $this->set(compact('metaDes', 'title_for_layout', 'metaKeys'));
    ...
    //function hotel($hotel_id)
       function hotel() {
        ...
        //debug($h); exit;
        $this->set(compact('pensions', 'h', 'title_for_layout', 'metaKeys', 'metaDes'));
    }
    ...
    function reserver($hotel_id, $chambre_id, $du, $au, $qte = 1) {
        ...
        $h = $this->Hotel->info_resa($hotel_id, $chambre_id);
        $dispo = $this->Inventaire->get_disponibilite(
                $hotel_id, $this->Session->read('Search.du'), $this->Session->read('Search.au')
        );
    ...
    function mtcUpdate() {
        $updateURL = "http://www.offredealshotels.com/mtcUpdate";
    ...
    function mtcBook() {
        $bookURL = "http://www.offredealshotels.com/mtcBook";
        $SLKSecretkey = Configure::read('mtcKey');
        $storeId = Configure::read('mtcStoreId');
        $cartId = $_POST["cartId"];
        $email = $_POST["email"];
